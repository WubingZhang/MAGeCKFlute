...
EnrichedView=function(enrichment, main = NULL, color = "#3f90f7", termNum = 15,
  ...
  # Select subset of enrichment
  enrichment = enrichment[order(enrichment$logP, decreasing = TRUE), ]
  if(nrow(enrichment)>=termNum){
    enrichment=enrichment[1:termNum, ]
  }
  enrichment$Description=factor(enrichment$Description,levels=rev(enrichment$Description))

  p1 = ggplot(data=enrichment,aes(x=logP, y=Description, size=Count))
...
#' }
#' @export
EnrichedGSEView = function(enrichment, top = 5, bottom = 5, charLength = 40,
  ...
  enrichment$col = "top"
  enrichment$col[enrichment$NES<0] = "bottom"
  p1 <- ggplot(data=enrichment, aes(x=NES, y=Description, size=logP))
  p1 <- p1 + geom_point(aes(color = col))
  p1 <- p1 + scale_color_manual(values = c("#377eb8", "#e41a1c"))
  p1 <- p1 + theme(panel.grid.major=element_line(colour="gray90"),
                 panel.grid.minor=element_blank(),
                 panel.background=element_blank())
  p1 <- p1 + xlab("Enrichment score") + ylab(NULL)
  p1 <- p1 + labs(title=main)
  p1 = p1 + theme(legend.position="right")
  p1 = p1 + theme(legend.key = element_rect(fill = "transparent", colour = "transparent"))
  p1 = p1 + theme(text = element_text(colour="black",size = 14, family = "Helvetica"),
